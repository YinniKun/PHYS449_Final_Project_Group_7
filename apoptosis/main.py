...
def update_p_vals(data_class, model, p):
    ...
    inner_loss = np.zeros(8)  # this holds the inner losses for each s species
    grad_update = np.zeros(9)  # how much to update each value of p
    for t_ind, t in enumerate(data_class.ode_inputs):
        f = np.asarray(get_data.apoptosis_model([t], p, states_np[t_ind]))
    ...
    for t_ind, t in enumerate(data_class.ode_inputs):
        f = np.asarray(get_data.apoptosis_model([t], p, states_np[t_ind]))
        grad_f = np.asarray(get_data.grad_apoptosis_model(t, p, states_np[t_ind]))
...
def run_nn(param, model, data):
    ...
    # concentration indexes for apoptosis model (0 indexed)
    m = [3]
